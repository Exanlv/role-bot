...
export class SelfAssignEmotesTest extends BaseTest implements UnitTest {
	...
	public async runTests() {
		...
		await this.runCommand(`!rbt sa c a ${this.categoryName}`);
		await this.runCommand(`!rbt sa c a ${this.secondCategoryName}`);
		...
		await this.runCommand(`!rbt sa r a r:{${this.testingRole.name}} c:{${this.categoryName}}`);
		...
		await this.simpleTest(
			`!rbt sa e l r:{${this.testingRole.name}} c:{${this.categoryName}}`,
		...
		await this.simpleTest(
			`!rbt sa e a r:{${this.testingRole.name}} c:{${this.fakeCategory}}`,
		...
		await this.simpleTest(
			`!rbt sa e a r:{${this.testingRole.name}} c:{${this.secondCategoryName}}`,
		...
		await this.simpleTestDoubleResponse(
			`!rbt sa e a r:{${this.testingRole.name}} c:{${this.categoryName}}`,
			'``> enter message link``',
		...
		await this.simpleTest(
			`!rbt sa e a r:{${this.testingRole.name}} c:{${this.categoryName}} HTTPS://DISCORDAPP.COM/CHANNELS/whatever`,
			'Could not add reaction role assignment; no valid message link was given',
			'Adding react invalid message url'
		);

		await this.simpleTest(
			`!rbt sa e a r:{${this.testingRole.name}} c:{${this.categoryName}} HTTPS://DISCORDAPP.COM/CHANNELS/guild/channel/message`,
			'Could not add reaction role assignment; message is not on this guild',
			'Adding react message url not on guild'
		);

		await this.simpleTest(
			`!rbt sa e a r:{${this.testingRole.name}} c:{${this.categoryName}} HTTPS://DISCORDAPP.COM/CHANNELS/${this.testServer.id}/channel/message`,
		...
		await this.simpleTest(
			`!rbt sa e a r:{${this.testingRole.name}} c:{${this.categoryName}} HTTPS://DISCORDAPP.COM/CHANNELS/${this.testServer.id}/${this.testChannel.id}/message`,
		...
		await this.simpleTest(
			`!rbt sa e a r:{${this.testingRole.name}} c:{${this.categoryName}} ${message.url}`,
		...
		await this.simpleTestDoubleResponse(
			`!rbt sa e a r:{${this.testingRole.name}} c:{${this.categoryName}} ${this.testingMessage.url}`,
		...
		await this.simpleTestList(
			`!rbt sa e l c:{${this.categoryName}} r:{${this.testingRole.name}}`,
			[
				{},
		...
		await this.simpleTest(
			`!rbt sa e r c:{${this.fakeCategory}}`,
		...
		await this.simpleTestDoubleResponse(
			`!rbt sa e r c:{${this.categoryName}}`,
			'``> enter role name``',
		...
		await this.simpleTestDoubleResponse(
			`!rbt sa e r c:{${this.categoryName}} r:{${this.testingRole.name}}`,
		...
		await this.simpleTest(
			`!rbt sa e r c:{${this.categoryName}} r:{${this.testingRole.name}} rm:{1}`,
			`Succesfully removed react(s) \`\`1\`\` from \`\`${this.testingRole.name}\`\` in category \`\`${firstLetterUppercase(this.categoryName)}\`\``,
			'Removing react'
		);
	}

	private async addReactTest(testCode) {
		const respondedMessage = await this.runCommand(`!rbt sa e a r:{${this.testingRole.name}} c:{${this.categoryName}} ${this.testingMessage.url}`) as Message;
