...
export class SelfAssignCategoryTest extends BaseTest implements UnitTest {
	...
	public async runTests() {
		...
		await this.simpleTest(
			'!rbt sa c a abcdefghijklmnopqrstuvwxyzabcdefghijklmnopqrstuvwxyz',
		...
		await this.simpleTestDoubleResponse(
			`!rbt sa c c on:{${this.categoryName}}`,
		...
		await this.simpleTest(
			`!rbt sa c c on:{${this.categoryName}} nn:{abcdefghijklmnopqrstuvwxyzabcdefghijklmnopqrstuvwxyz}`,
		...
		await this.simpleTest(
			`!rbt sa c c on:{${this.categoryName}} nn:{${this.secondCategoryName}}`,
		...
		await this.simpleTest(
			`!rbt sa c c on:{${this.categoryName}} nn:{${this.newCategoryName}}`,
		...
		await this.simpleTest(
			`!rbt sa c s c1:{${this.fakeCategoryName}}`,
		...
		await this.simpleTestDoubleResponse(
			`!rbt sa c s c1:{${this.newCategoryName}}`,
		...
		await this.simpleTest(
			`!rbt sa c s c1:{${this.newCategoryName}} c2:{${this.fakeCategoryName}}`,
			`Could not swap categories; category \`\`${firstLetterUppercase(this.fakeCategoryName)}\`\` does not exist`,
			'Swapping category 2nd category doesnt exist'
		);

		await this.simpleTest(
			`!rbt sa c s c1:{${this.newCategoryName}} c2:{${this.secondCategoryName}}`,
