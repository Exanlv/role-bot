...
import { Collection, DiscordAPIError, Emoji, GuildChannel, Message, MessageReaction, Role, TextChannel, User } from 'discord.js';

import * as fetch from 'node-fetch';

export class AddEmoteCommand extends BaseCommand implements IBaseCommand {
	...
	public async runCommand(): Promise<void> {
		...
		const role = await this.message.guild.roles.find((r: Role) => r.name.toUpperCase() === roleName);
		...
		let messageLink = this.args.find((a: string) => a.startsWith('HTTPS://DISCORDAPP.COM/CHANNELS/'));
		...
		const channel = this.message.guild.channels.find((c: GuildChannel) => c.id === channelId) as TextChannel;
		...
		const reactMessage = await channel.fetchMessage(messageId).catch((err: any) => {
			return;
		});
		...
		this.message.channel.send('Please add the react for the role to this message').then((message: Message) => {
			message = message as Message;
			message.awaitReactions((reaction: MessageReaction, user: User) => user.id === this.message.author.id, {max: 1, time: 30000}).then(async (collection: Collection<string, MessageReaction>) => {
				...
				if (reactedEmote.id && !this.client.emojis.find((e: Emoji) => e.id === reactedEmote.id)) {
				...
				reactMessage.react(emoteToReact).then((e: MessageReaction) => {
	...
	public async uploadEmote(emoji: Emoji): Promise<Emoji|void> {
		const gifExists = (await fetch(`https://cdn.discordapp.com/emojis/${emoji.id}.gif`, {method: 'HEAD'})).status === 200;
		const url = `https://cdn.discordapp.com/emojis/${emoji.id}.${gifExists ? 'gif' : 'png'}`;
		const emote = await this.globalConfig.devServer.createEmoji(url, emoji.name).catch((e: DiscordAPIError) => {
	...
	public async deleteEmote(emoji: Emoji): Promise<void> {
		if (emoji.guild) {
			await this.globalConfig.devServer.deleteEmoji(emoji).catch((e: DiscordAPIError) => {
				return;
			});
		}
	}
